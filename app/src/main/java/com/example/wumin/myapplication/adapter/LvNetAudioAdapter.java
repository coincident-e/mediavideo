 ...
 */
public class LvNetAudioAdapter extends BaseAdapter {
    private Context context;
    private List<NetAudioBean.ListBean> data;
    private Utils utils;
    ...
    public LvNetAudioAdapter(Context context, List<NetAudioBean.ListBean> data) {
    ...
    public int getItemViewType(int position) {
        int result = -1;
        NetAudioBean.ListBean item = data.get(position);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            //绑定数据
            //根据位置得到数据,绑定数据
            NetAudioBean.ListBean mediaItem = getItem(position);
    ...
    private void bindData(int itemViewType, ViewHolder viewHolder, NetAudioBean.ListBean mediaItem) {
        switch (itemViewType) {
            case TYPE_VIDEO://视频
                ...
                viewHolder.jcv_videoplayer.setUp(mediaItem.getVideo().getVideo().get(0), JCVideoPlayer.SCREEN_LAYOUT_LIST, mediaItem.getVideo().getThumbnail().get(0), null);
...
//viewHolder.tv_commant_context.setText(mediaItem);
                break;
            case TYPE_IMAGE://图片
                bindData(viewHolder, mediaItem);
                viewHolder.iv_image_icon.setImageResource(R.drawable.bg_item);
                System.out.println("------------image height"+mediaItem.getImageBean().getHeight());
                int  height = mediaItem.getImageBean().getHeight()<= DensityUtil.getScreenHeight()*0.75?mediaItem.getImageBean().getHeight(): (int) (DensityUtil.getScreenHeight() * 0.75);
                ...
                if(mediaItem.getImageBean() != null &&  mediaItem.getImageBean().getBig()!= null&&mediaItem.getImageBean().getBig().size() >0){
//                    x.image().bind(viewHolder.iv_image_icon, mediaItem.getImageBean().getBig().get(0));
                    Glide.with(context).load(mediaItem.getImageBean().getBig().get(0)).placeholder(R.drawable.bg_item).error(R.drawable.bg_item).diskCacheStrategy(DiskCacheStrategy.ALL).into(viewHolder.iv_image_icon);
                }
                break;
            case TYPE_TEXT://文字
                bindData(viewHolder, mediaItem);
    ...
    private View initView(View convertView, int itemViewType, ViewHolder viewHolder) {
        switch (itemViewType) {
            ...
            case TYPE_AD://软件广告
                ...
                viewHolder.iv_image_icon = (ImageView) convertView.findViewById(R.id.iv_image_icon_ad);
                break;
        }
        return convertView;
    }
